...
class ModelArguments:
    ...
    # Isotropy's arguments
    loss_fn_settings: Optional[str] = field(
        default=None,
        metadata={
            "help": "What kind of loss function to use."
        }
    )
    loss_param_settings: Optional[str] = field(
        default=None,
        metadata={
            "help": "Parameters in loss function."
        }
    )
    mt_size: Optional[int] = field(
        default=None,
        metadata={
            "help": "Embedding size for mt sub-space."
        }
    )
...
class OurDataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        # input_ids, attention_mask, token_type_ids：[Tensor of shape(abs, asn, seq_len), Tensor of shape(obs, osn, seq_len)]
        ...
        ''' 
        common:
            key -> [tensor of shape(aigen_batch_size, aigen_sent_num, seq_len),
                    tensor of shape(other_batch_size, other_sent_num, seq_len)]
        unsup:
            key -> [tensor of shape(0, 0, seq_len),
                    tensor of shape(batch_size, 2, seq_len)]
        sup:
            key -> [tensor of shape(0, 0, seq_len),
                    tensor of shape(batch_size, 3, seq_len)]
        '''
        return batch
